...
> **对局运行循环**（凑人开局 → 倒计时 → 波次掷骰 → 淘汰旁观 → 结算 → 关房间）已就位。
>
> **对局已经能完整跑完一局。** 五个语义事件（开局 / 灾难生成 / 玩家淘汰 / 方块变更 / 对局结束）
> 全部开始派发，结算会写入存储。
> **但灾难的实际效果只做了地陷一个**——其余 9 个灾种掷中后只落点、不动方块，
> 服主可以用 `/ds info` 看当前实现了几个。
...
**目前只有地陷实现了效果**，其余灾种会照常掷骰、照常落点、照常广播事件，就是不动方块。
`/ds info` 里能看到当前实现了哪几个。
...
房间的状态与分流、掷骰与落点校验、地陷形状、对局胜负判定都是纯逻辑。凡是
「编译期看不出来、只会在服主机器上炸」的东西（文件格式、防连刷、降级路径、状态机）
都应当补一条回归。

目前的覆盖共 **144 例**（15 个测试类）：`disaster` 53、`map` 19、`room` 43、
