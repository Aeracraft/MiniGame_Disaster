...
| `/ds rate <1-5> [标签]` | `disaster.play` | 给刚打完的一局打分，见「对局评价」 |
| `/ds rep <玩家> [标签]` | `disaster.play` | 给同局玩家点赞；不带参数看自己的荣誉值 |
...
结算之后默认 **五分钟**内（`rating.window-seconds`），参与者可以做两件事：

| 命令 | 作用 |
|---|---|
| `/ds rate <1-5> [标签…]` | 匿名给整局打分，标签从 `rating.tags` 里挑 |
| `/ds rep <玩家> [标签…]` | 给同局玩家点赞，标签从 `rating.rep-tags` 里挑；不带参数时看自己的荣誉值 |

不用手输局标识：结算时插件给参与者开了一扇窗，`/ds rate` 评的就是窗口里那一局。
房间在结算十秒后就销毁了，而窗口独立于房间存活，所以五分钟是够用的。

**两项都不收自由文本。** 标签只能从配置里的固定选项挑，填别的会被拒——没有文本就没有
辱骂与灌水，也省掉一整套内容审核。
...
| 参与者校验 | 不在本局的人不能评 |
| 禁止自评 | 互评不能指向自己 |
| 一局一次 | 评价一局一人一次；互评一局对一人一次 |
| 冷却 | 同一对玩家两次互评之间默认隔 7 天（`rating.cooldown-days`） |
| 时间窗 | 窗口一过，本局不再受理 |
| 无自由文本 | 标签必须来自固定选项 |

每人每局最多赞几个人由 `rating.per-match-limit` 控制（默认 3，0 表示不限）。被拒时会
直接告诉玩家是哪一条拦下的，不会只回一句「失败」。

### 存储与可插拔

两个子系统的数据都走同一个 `StorageProvider`——单机用 YAML，多子服共用 MySQL 时评价自动跨服可见。

互评实现可替换，三个插入点：

1. **内置实现**：走 `StorageProvider` 落库，开箱即用。
2. **注册到 `ServicesManager`**：实现 `ReputationProvider` 并注册即替换内置实现；注册了但
   自报不可用（比如它自己的存储没起来）会退回内置，而不是让评价整个消失。
3. **事件旁听**：`DisasterMatchRatedEvent` 与 `DisasterPlayerRepEvent`，只想记录不想接管时走这条。

有两道闸由插件固定执行、不交给实现方：**评价时间窗与参与者校验**。它们属于游戏语义而非
存储策略。其余几道（内容合法性、重复、冷却、每局配额）由实现方自行决定。
...
**当前状态**：上报器尚未实装。上面这套是设计基准，`config.yml` 里的 `webhook` 段目前
配了也不会有推送，它排在下一个里程碑。与本插件其余「只出不进」的性质无关——插件现在
同样不监听任何端口。
...
│   ├── reputation/            互评契约（ReputationProvider）与受理结果
...
├── reputation/                对局评价与互评：评价窗口、六道防刷闸、内置实现与编排
...
房间的状态与分流、掷骰与落点校验、灾种形状与路线、对局胜负判定、评价窗口与六道防刷闸
都是纯逻辑。凡是「编译期看不出来、只会在服主机器上炸」的东西（文件格式、防连刷、
降级路径、状态机）都应当补一条回归。

目前的覆盖共 **177 例**（20 个测试类）：`disaster` 68、`room` 43、`map` 19、
`reputation` 18、`match` 14、`permission` 9、`world` 6。随机相关的用例都用固定种子，
失败可以直接重演。
